...
MYSQL_ROW row;
int i, fields;
sprintf(query, "select * from info");
...
if (!mysql_real_connect(&mysql, "localhost", "root", "1234", "user", 0, (char *)NULL, 0)) {
printf("MySQL Error %d: %s\n", mysql_errno(&mysql), mysql_error(&mysql));
exit(1);
}
while(1){
	...
	switch(choice){
		case 1:
			//printf("you want to select TELL\n");
			//printf("\nTELL : ");
			//scanf("%s", tell);
		...
		case 2:
			...
			printf("\nADDRESS : ");
			scanf("%s", address);
			sprintf(query, "insert into info values(%d, \'%s\', \'%s\')", id, tell, address);
			if (mysql_query(&mysql, query)) {
				printf("MySQL Error %d: %s\n", mysql_errno(&mysql), mysql_error(&mysql));
				exit(1);
			}
		...
		case 3:
			...
			printf("\nADDRESS : ");
			scanf("%s", address);
			sprintf(query, "update info set tell='%s', address='%s' where id='%d'", tell, address, id);
			if (mysql_query(&mysql, query)) {
				printf("MySQL Error %d: %s\n", mysql_errno(&mysql), mysql_error(&mysql));
				exit(1);
			}
